...
Mat img; // lưu hình ảnh đầu vào

// OpenCV command line parser functions. Keys accepted by command line parser
const char *keys = {
...
void showHistoCallback(int state, void *userData)
{
    ...
    // Create the histogram for 256 bins, the number of possible values
    int numbins = 256;
    ...
    // Draw the histogram. We go to draw lines for each channel
    int width = 512, height = 300;

    // Create image with graybase
...
void lomoCallback(int state, void *userData) {
    ...
    for (int i = 0; i < 256; ++i) {
        float x = (float)i / 256.;
        lut.at<uchar>(i) = cvRound(256*(1/(1 + pow(E, -((x - .5)/.1)))));
    }
    ...
    split(img, bgr);
    LUT(bgr[2], lut, bgr[2]);
    merge(bgr, result); // merge result

    // Create image for halo dark
    Mat halo(img.rows, img.cols, CV_32FC3, Scalar(.3, .3, .3));
...
void cartoonCallback(int state, void *userData) {
    ...
    Mat imgBF;
    bilateralFilter(img, imgBF, 9, 150., 150.);
